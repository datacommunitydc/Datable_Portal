...
def test_Tfidf():
    ...
    # between them. Any textbook on information retrieval (IR) covers this. See esp.
    # Introduction to Information Retrieval, which is free and available online.

    #get_two_string_sim(v_1, v_2)

    from sklearn.feature_extraction.text import TfidfVectorizer

    vect = TfidfVectorizer(min_df=1)
    test_text1 = ["I'd like an apple",
                  "An apple a day keeps the doctor away",
                  "Never compare an apple to an orange",
                  "I prefer scikit-learn to Orange"]
    tfidf = vect.fit_transform(test_text1)
...
def main():
    ...
    # plt.interactive(False) #for PyCharm displaying
